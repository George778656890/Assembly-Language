;Lab Assignment 5---Part2
;Author:Zhihong  Liu(5088806)
;16/10/2024
;Compute  Fibonacci numbers and store them in an array using pointer method 
...
ptrArray DWORD offset array ;pointer for the array
...
mov esi,ptrArray        ;Initialize the register
mov DWORD PTR [esi],0   ;Store the 0th Fibonacci number into the array
add esi,TYPE array
mov DWORD PTR [esi],1   ;Store the 1st Fibonacci number into the array
add esi,TYPE array
mov DWORD PTR [esi],1   ;Store the 2nd Fibonacci number into the array
...
mov DWORD PTR[esi],ebx  ;Store the Fibonacci number to the array
add esi,TYPE array      ;Move to the next index

LOOP FibonacciLoop 

mov ecx,count            ;Printing loop counter  
mov esi,ptrArray         ;Reset the ESI to the pointer 1st address
